...
SoftwareSerial esp_serial(9, 8);
Esp32::Wifi wifi(&esp_serial);
Esp32::Mqtt mqtt(&esp_serial);

// mqtt.publish_message("V1", String("xin chao").c_str());

void on_receive_V1()
{
  // if (String(mqtt.getMessageFrom("button")[0]) == "0")
  // {
  //   state = INIT;
  // }
  // else
  // if (String(mqtt.getMessageFrom("button")[0]) == "1")
  // {
  //   state = READY_AUTO;
  // }
  // else if (String(mqtt.getMessageFrom("button")[0]) == "2")
  // {
  //   state = PUMPING_AUTO;
  // }
  // else if (String(mqtt.getMessageFrom("button")[0]) == "3")
  // {
  //   state = STOP_PUMPING_AUTO;
  // }
  // else if (String(mqtt.getMessageFrom("button")[0]) == "4")
  // {
  //   state = PUMPING_MAN;
  // }
  // else if (String(mqtt.getMessageFrom("button")[0]) == "5")
  // {
...
void setup()
{
  ...
  wifi.restart();
  wifi.connect_to_ap("Hung Tran", "hungtran260902");
  mqtt.connect_mqtt_broker("io.adafruit.com", 1883, "Suchuru", "aio_rctQ44tIQlubyfnpLYr68iav0PB8");
  mqtt.subcribe_topic("button", on_receive_V1);
...
String check_4Pumping(float Moisture_value, int SensorNum, int PinRelay)
{
  ...
  if (Moisture_value >= 0.0 && Moisture_value <= 3.75)
  { // 2.75
    Serial.println(PUMP);
    return PUMP;
  }
...
void display_LCD()
{
  if (timer2_flag == true)
  {
    ...
    mqtt.publish_message("moisturev1", Moisture_Sensor1);
    mqtt.publish_message("moisturev2", Moisture_Sensor2);
  ...
  case READY_AUTO:
    ...
    if (timer1_flag == true)
    {
      ...
      if (pump1_state == PUMP || pump2_state == PUMP)
      {
        ...
        mqtt.publish_message("button", "2");
      }
      else
      {
        state = STOP_PUMPING_AUTO;
        setTimer1((uint64_t)time_no_pump * 1000);
        mqtt.publish_message("button", "3");
      }
    }
    break;
  case PUMPING_AUTO:
